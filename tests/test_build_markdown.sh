  ...
  cat > "$pred" <<'JSON'
{
  ...
  "merge": {
    ...
    "merged_by": "SaharBracha",
    "target_branch": "main",
    "target_base_sha": "ed6de7af"
  },
  "approvers": [
    { "login": "yurinovo18", "submitted_at": "2026-07-23T11:27:58Z", "is_pr_head_approval": true }
  ],
  "commits_on_target_branch": ["55bf3c3a", "0b2b9eec"],
  "code_committers": [
    { "login": null, "email": "saharb@jfrog.com" }
  ],
  "collection": {
    "collected_at": "2026-07-23T11:28:30.000Z",
    "workflow_run_url": "https://github.com/SaharBracha/github-evidence/actions/runs/30003234739"
  }
}
JSON

  cat > "$subj" <<'JSON'
{ "head_sha": "0b2b9eec", "pr_number": "7", "head_ref": "feature-x", "repo_url": "https://github.com/SaharBracha/github-evidence", "created_at": "2026-07-23T11:28:30.000Z" }
JSON
  ...
  assert_contains "$out" "**Repository:** SaharBracha/github-evidence" "repository slug from subject" || return 1
  assert_contains "$out" "**Pull request:** #7" "pr number" || return 1
  assert_contains "$out" "**Merged by:** SaharBracha" "merged_by" || return 1
  assert_contains "$out" "ed0be7a1" "merge commit sha" || return 1
  assert_contains "$out" "Approvers (1)" "approver count" || return 1
  assert_contains "$out" "yurinovo18" "approver login" || return 1
  ...
  assert_contains "$out" "mailto:saharb@jfrog.com" "committer email as mailto link" || return 1
...
{
  "schema_version": "1.0",
  "repository": { "full_name": "SaharBracha/github-evidence", "url": "https://github.com/SaharBracha/github-evidence" },
  "collection": { "collected_at": "2026-07-23T11:28:24Z", "workflow_run_url": "https://github.com/SaharBracha/github-evidence/actions/runs/1" },
