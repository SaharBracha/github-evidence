...
test_sign_dsse_envelope_shape_and_verify() {
  ...
  payload_b64="$(printf '{"_type":"https://in-toto.io/Statement/v1","subject":[{"name":"gitCommit","digest":{"gitCommit":"abc123"}}]}' | base64 | tr -d '\n')"
  jq -n \
    --arg payload "$payload_b64" \
    '{dsse_payload: $payload, dsse_payload_type: "application/vnd.in-toto+json", post_url: "/evidence/api/v1/entity/gitCommit/abc123"}' \
    > "${tmp}/prep.json"
